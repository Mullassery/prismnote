      ...
      <div className="flex-1 overflow-y-auto p-4 space-y-4 max-w-3xl">
        ...
        <div>
          ...
          <div className="flex gap-2">
            <input value={dir} onChange={(e) => setDir(e.target.value)} placeholder="/Users/you/my-notebooks"
              className="flex-1 px-2 py-1.5 rounded bg-white/5 border pn-bd pn-text text-[13px] outline-none focus:border-blue-500" />
        ...
        {status && (
          <div className="rounded-lg border pn-bd pn-solid-bg p-3 text-[13px]">
            {status.is_repo ? (
              <>
                <div className="flex items-center gap-2 pn-text"><GitBranch size={13} className="text-blue-400" /> {status.branch || 'detached'}</div>
              ...
              <div className="flex items-center justify-between">
                ...
                <button onClick={() => run(() => gitInit(dir))} disabled={!dir || busy} className="flex items-center gap-1 px-2 py-1 rounded bg-blue-500/20 text-blue-200 text-[12px]"><FolderGit2 size={13} /> git init</button>
              </div>
            )}
          </div>
        )}
        ...
        <div className="rounded-lg border pn-bd p-3 space-y-2">
          ...
          <input ref={commitRef} value={message} onChange={(e) => setMessage(e.target.value)} placeholder="Commit message"
            className="w-full px-2 py-1.5 rounded bg-white/5 border pn-bd pn-text text-[13px] outline-none focus:border-blue-500" />
          <div className="flex gap-2">
